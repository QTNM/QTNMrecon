 ...
 *
 */
// reduced features: YR, Universoty of Warwick, 2024
...
void spline::info() const
{
    std::cout << "type " << m_type << ", left boundary deriv " << m_left << " = ";
    std::cout << m_left_value << ", right boundary deriv " << m_right << " = ";
    std::cout << m_right_value << std::endl;
    if(m_made_monotonic) {
        std::cout << "(spline has been adjusted for piece-wise monotonicity)";
    }
}
