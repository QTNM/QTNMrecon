...
QTNMSimHitReader::QTNMSimHitReader(std::string fname, std::string out) : 
    outkey(std::move(out)),
    maxEventNumber(-1), // default -1 for a all events
    evcounter(0)
{
    // prepare the input file, read-only
    try {
    ...
    reader = new TTreeReader("ntuple/Score", fin); // takes pointer to TFile
    // reads QTNMSim output file with given ttree name
}

QTNMSimHitReader::~QTNMSimHitReader()
{
    fin->Close();
}
...
Event_map<std::any> QTNMSimHitReader::operator()()
{
    ...
    if (evcounter >= maxEventNumber && maxEventNumber > 0) // max has been set
