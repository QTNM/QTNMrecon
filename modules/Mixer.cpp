...
DataPack Mixer::operator()(DataPack dp)
{
    ...
    if (dp.getTruthRef().tooShort) {
      std::cout << "Waveform too short to process: Mixer." << std::endl;
      return dp;
    }
    ...
    demod.setFilterCutoff(cutFrequency);
	...
	//	std::cout << "mixer module " << losc << " losc freq set" << std::endl;
	    ...
	    //	    std::cout << "mixer module " << ikey << " retrieved size " << pure.size() << std::endl;
